<aside class="w-64 shrink-0 bg-ink text-white flex flex-col">
    ...
    <template x-for="item in navItems" :key="item.id">
      <a
        ...
        <span x-show="item.badge" x-text="item.badge"
              class="ml-auto text-[11px] font-mono px-1.5 py-0.5 rounded"
              :class="activeTab === item.id ? 'bg-ink/20 text-ink' : 'bg-white/10 text-white/70'"></span>
      </a>
    </template>
  </nav>
